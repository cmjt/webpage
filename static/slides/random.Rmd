...
$\mu = \int_{{\rm I\!R^{2+1}}} \Lambda(\boldsymbol{x})dx$ where $$\Lambda(\boldsymbol{x}) = \textrm{exp}(\bf{X}\beta + Z(\boldsymbol{x}) + \eta(\boldsymbol{x})).$$
<br>
 <br>
...
--
 ...
 - But surely we should have started with the why `r fontawesome::fa("question-circle")`
...
$$\begin{array}{l}
    \text{log}(\Lambda(\boldsymbol{x})) = \bf{X}\beta + Z(\boldsymbol{x}) \\ 
     g(m_1(\boldsymbol{x})) = \tilde{\bf{X}\beta} + \gamma_1\: Z(\boldsymbol{x})  + G(\boldsymbol{x})\\
      h(m_2(\boldsymbol{x})) = \tilde{\tilde{{\bf{X}\beta}}} + \gamma_2\:Z(\boldsymbol{x}) + K(\boldsymbol{x}).\\
    \end{array}$$
...
.pull-right[
<br>
.center[`r fontawesome::fa("question-circle")` UFO sightings]
<br>
