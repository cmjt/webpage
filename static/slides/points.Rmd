...
### `r emo::ji("palm_tree")` The Palm intensity function 

 + The Palm intensity, $\lambda(r; \theta)$, describes how the point density varies as a function of distance $r$ from an arbitrarily chosen point. 
...
```{r plot weights,echo = FALSE, fig.width=8,fig.height=8}
...
grfs <- fields[[1]]
stelfi::show_field(int <- grfs, mesh, dims = c(300,300),
                   col = RColorBrewer::brewer.pal(9, "Blues"),
                   sp = nz,zlim = range(int),asp = 1)
```
...
```{r murders_cov, fig.align='center',fig.width=8,fig.height=8, eval = TRUE}
...
fields <- stelfi::get_fields(fit, mesh, mean = TRUE)
grfs <- fields[[1]]
stelfi::show_field(grfs, mesh, dims = c(300,300),
                   col = RColorBrewer::brewer.pal(9, "Blues"),
                   sp = nz, zlim = range(int),asp = 1)
```
